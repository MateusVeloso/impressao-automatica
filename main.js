...
function createWindow() {
    ...
    mainWindow.webContents.getPrinters().forEach(function(entry) {
        if (entry.isDefault) {
            impressora = entry.name;
        }
    });
...
ipc.on('imprimir', function(event, arg) {
    mainWindow.webContents.print({ silent: true, printBackground: true, deviceName: impressora }, () => {
        win.loadFile('contrato-frete.html');
    });
});
